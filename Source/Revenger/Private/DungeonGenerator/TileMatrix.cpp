...
#include "DungeonGenerator/TileMatrix.h"
...
void FTileMatrix::CreateRooms(int32 RoomCount)
{
    ...
    for (int32 i = 0; i < RoomCount; i++) {
        ...
        // Generate rooms based on random numbers
...
void FTileMatrix::CreateStartRoom(int16 Rows, int16 Columns)
{
  for (int32 i = 0; i < Rows; i++) {
        // Generate rooms based Rows and Columns
        for (int32 j = 0; j < Columns; j++) {
            int32 RoomSize = FMath::RandRange(MinRoomSize, MaxRoomSize);
            TArray<Tile> RoomTiles;
            // Make start room 5x8
            if (CanPlaceRoomInTileMap(Tile(i, j), RoomSize, RoomTiles)) {
                // Occupy tiles
                for (int32 k = 0; k < RoomTiles.Num(); k++) {
                    OccupyTile(RoomTiles[k]);
                }
